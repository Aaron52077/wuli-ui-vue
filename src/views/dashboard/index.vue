<template>
    <div class="container">
        <div v-if="selected === '1'">
            <div class="doc-title wuli-hairline__bottom">wuli-ui 组件库</div>
            <div v-for="(group, index) of list" :key="index">
                <div class="wuli-panel__title">{{ group.title }}</div>
                <div class="wuli-panel">
                    <router-link v-for="(item, idx) of group.content" :key="idx" class="wuli-cell wuli-cell__access" :to="item.path">
                        <div class="wuli-cell__bd">{{ item.name }}</div>
                        <div class="wuli-cell__ft"></div>
                    </router-link>
                </div>
            </div>
        </div>
        <div v-else>
            <div class="doc-title wuli-hairline__bottom">STAR</div>
            <div class="doc-description">
                <img class="doc-description__img" src="data:image/png;base64," />
                <span>https://github.com/Aaron52077/wuli-ui-vue</span>
            </div>
            <div class="wuli-panel__title">Easy Mock接口测试</div>
            <div class="wuli-panel">
                <div class="wuli-btns">
                    <wp-button type="primary" size="small" @click="clickHandle" round>Easy Mock</wp-button>
                </div>
            </div>
            <template v-if="mockList.length != 0">
                <div class="wuli-panel">
                    <wp-cell v-for="item of mockList" :key="item.age" :title="'名字：'+ item.name" :value="'年龄：'+ item.age"></wp-cell>
                </div>
            </template>
        </div>
        
        <!-- 底部导航 -->
        <wp-tabbar v-model="selected">
            <wp-tabbar-item id="1">
                <img src="~@/assets/images/icon-fun.png" slot="normal"> 
                <img src="~@/assets/images/icon-fun-selected.png" slot="active"> 
                <span>组件类</span>
            </wp-tabbar-item>
            <wp-tabbar-item id="2">
                <img src="~@/assets/images/icon-person.png" slot="normal"> 
                <img src="~@/assets/images/icon-person-selected.png" slot="active"> 
                <span>个人</span>
            </wp-tabbar-item>
        </wp-tabbar>
    </div>
</template>

<script>
import components from './config'
import wpTabbar from '@/components/tabbar'
import wpTabbarItem from '@/components/tabbar/item'
import { getMock } from '@/api'

export default {
    data() {
        return {
            list: components,
            selected: '1',
            mockList: []
        }
    },
    methods: {
        clickHandle() {
            getMock().then(res => {
                this.mockList = res.data.test
            })
        }
    },
    components: { wpTabbar, wpTabbarItem },
    watch: {
        selected: {
            handler: function(val, oldVal) { 
                 
            },
            deep: true
        }
    }
}
</script>
